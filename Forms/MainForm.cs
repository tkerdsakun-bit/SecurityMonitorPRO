    ...
    private void CreateApplicationFolders()
{
    try
    {
        ...
        if (!Directory.Exists(quarantinePath))
        {
            ...
            Log($"Created Quarantine folder: {quarantinePath}", LogLevel.Info);  // ✅ เปลี่ยนจาก Console.WriteLine
        }
        ...
        if (!Directory.Exists(logsPath))
        {
            ...
            Log($"Created Logs folder: {logsPath}", LogLevel.Info);  // ✅ เปลี่ยนจาก Console.WriteLine
        }
        ...
        if (!Directory.Exists(dbPath))
        {
            ...
            Log($"Created Database folder: {dbPath}", LogLevel.Info);  // ✅ เปลี่ยนจาก Console.WriteLine
        }
        
        Log("All application folders verified/created successfully", LogLevel.Success);  // ✅ เปลี่ยนจาก Console.WriteLine
    }
    catch (Exception ex)
    {
        Log($"Error creating application folders: {ex.Message}", LogLevel.Error);  // ✅ เพิ่ม Log
        MessageBox.Show(
...
private void ColorizeLogsInRichTextBox(RichTextBox rtb, string[] logs)
{
    ...
    foreach (var log in logs)
    {
        ...
        // ✅ ใช้ Contains และตรวจแบบ case-insensitive
        if (log.Contains("[INFO", StringComparison.OrdinalIgnoreCase))
            logColor = Color.Cyan;
        ...
        else if (log.Contains("[DEBUG", StringComparison.OrdinalIgnoreCase))
            logColor = Color.Gray;
...
private (int Total, int Info, int Warning, int Error, int Success, int Debug) GetLogStatistics(string[] logs)
{
    ...
    // ✅ ใช้ Contains แทน เพื่อให้จับได้ทั้ง [INFO] และ [INFO   ]
    var info = logs.Count(l => l.Contains("[INFO", StringComparison.OrdinalIgnoreCase));
    var warning = logs.Count(l => l.Contains("[WARNING", StringComparison.OrdinalIgnoreCase));
    var error = logs.Count(l => l.Contains("[ERROR", StringComparison.OrdinalIgnoreCase));
    var success = logs.Count(l => l.Contains("[SUCCESS", StringComparison.OrdinalIgnoreCase));
    var debug = logs.Count(l => l.Contains("[DEBUG", StringComparison.OrdinalIgnoreCase));
